...
这是一个简洁且易于修改的 ImageNet 验证脚本，用于根据 ImageNet 或类似组织的图像数据集评估预训练模型或训练检查点。
模型或训练检查点。它优先考虑
标准 Python 风格和良好的性能。您可以根据自己的需要重新使用。
...
parser.add_argument('--pin-mem', action='store_true', default=False,
                    help='在数据加载器中引脚 CPU 内存，以便更高效地（有时）传输到 GPU。')
...
def validate(args):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                ...
                output_list.append([output.cpu().numpy().shape, target.cpu().numpy().shape])
                
                print(output.cpu().numpy().shape, target.cpu().numpy().shape)
    ...
    #np.save('output_list.csv', output_list)
